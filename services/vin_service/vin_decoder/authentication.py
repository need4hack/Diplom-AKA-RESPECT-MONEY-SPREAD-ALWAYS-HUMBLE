"""
Authentication backends for VIN Service.
...
class ServiceUser(models.Model):
    """Read-only proxy to the shared `users` table."""
...
class ServiceJWTAuthentication(JWTAuthentication):
    """Validate Bearer JWTs issued by auth_service and resolve shared users."""
...
class ApiKeyAuthentication(BaseAuthentication):
    """
    DRF authentication class for external API clients.
    ...
    def authenticate(self, request):
        ...
        logger.info(
            "VIN API key auth: user=%s, requests=%s/%s",
            user.username,
            user.request_count,
            user.request_limit,
        )
